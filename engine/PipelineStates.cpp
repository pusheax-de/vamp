...
bool PipelineStates::Init(ID3D12Device* device, const std::wstring& shaderDir)
{
    ...
    if (!CreateSpriteHardCutoutPSO(device, shaderDir))  return false;
...
bool PipelineStates::CreateSpriteHardCutoutPSO(ID3D12Device* device, const std::wstring& shaderDir)
{
    auto vs = CompileShader(shaderDir + L"/SpriteVS.hlsl", "main", "vs_5_1");
    auto ps = CompileShader(shaderDir + L"/SpriteHardCutoutPS.hlsl", "main", "ps_5_1");
    if (!vs || !ps) return false;
    ...
    return SUCCEEDED(device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_spriteHardCutoutPSO)));
}
